...
const HotelService = (id) => {
  ...
  useEffect(() => {
    if (transportDetail) {
      setTransport(transportDetail?.transport?.features || {});
      setServicesDet(transportDetail?.transportServiceIt || {});
      setActivities(transportDetail?.transportServiceIt?.services || []);
      setPriceInclude(transportDetail?.transportServiceIt?.priceIncludes || []);
      setPriceExclude(transportDetail?.transportServiceIt?.priceExcludes || []);
      setCompleteInfo(
        transportDetail?.transportServiceIt?.completeInfo || [{}]
      );
      console.log(transportDetail.transportServiceIt.completeInfo);
    }
  }, [transportDetail]);
          ...
          <div className="flex flex-col items-center justify-start w-full gap-y-6">
            ...
            <div className="flex flex-col items-center justify-center w-full gap-6 ssm:flex-row">
              ...
              <div className="bg-[#206eff] p-6 rounded-lg flex-1 shadow-lg border-2 border-[#206eff]">
                ...
                <ul className="text-white list-disc list-inside font-radios">
                  {Transport.length > 0 &&
                    Transport.map((item, index) => <li key={index}>{item}</li>)}
                </ul>
                <div className="mt-4">
                  <h3 className="text-[#206eff] bg-white p-3 text-lg font-radios w-max rounded-xl font-bold mb-4">
                    Price Excludes:
