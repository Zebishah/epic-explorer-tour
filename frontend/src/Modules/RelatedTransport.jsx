...
import { showRelatedTransport } from "../Redux/Slices/ShowTourDetailsSlice";
import { useNavigate } from "react-router";
import image2 from "../img/vecteezy_blue-and-white-curve-shape-background_11403724.jpg";
const RelatedTransport = (id) => {
  const dispatch = useDispatch();
  const [TransportDet, setTransportDet] = useState([]);
  const navigate = useNavigate();
  const { relatedTransport } = useSelector((state) => state.TourDetail);

  useEffect(() => {
    if (id) {
      dispatch(showRelatedTransport(id));
    }
  }, []);
  useEffect(() => {
    if (relatedTransport && relatedTransport.transport) {
      setTransportDet(relatedTransport.transport.slice(0, 3));
    }
    console.log(relatedTransport);
  }, [relatedTransport]);
  const BookTransport = (id) => {
            ...
            <div className="p-3 mt-2 bg-[#206eff] rounded-xl">
              ...
              <div className="flex flex-row justify-between mt-4 ">
                <h3 className="text-xs text-white font-radios">
                  <span className="text-sm font-radios">Seats : </span>
                  {tour.seats} seater
                </h3>
              </div>
...
export default RelatedTransport;
