...
const RequestTransportBalance = () => {
  ...
  const { extraTPaymentDetail } = useSelector((state) => state.TourDetail);
  useEffect(() => {
    if (id) {
      dispatch(showExtraTDetail(id));
    }
  }, [dispatch]);
  useEffect(() => {
    if (extraTPaymentDetail) {
      if (extraTPaymentDetail.transport) {
        console.log(extraTPaymentDetail);
        setAmount(extraTPaymentDetail.transport.prices);
        setAdmin(extraTPaymentDetail.admin);
        setTourName(extraTPaymentDetail.transport.carName);
        setTourId(extraTPaymentDetail.transport.transportId);
      }
    }
  }, [extraTPaymentDetail]);
  ...
  const ProceedPayment = async () => {
    ...
    try {
      ...
      if (data.success && data.success === true) {
        setLoading(false);
        toast.success("Transport successfully Booked");
        const timer = setTimeout(() => {
          navigate(`/TransportTicket?id=${TourId}`);
        }, 3000);
...
export default RequestTransportBalance;
