...
const SingleNavbar = () => {
  ...
  return (
    <div className="z-20 flex flex-col w-screen gap-y-1">
      <ToastContainer />
      <nav className="flex justify-between py-4 px-4 bg-[#ffffffd5] opacity-100 fixed w-full left-0 right-0 z-30">
        {" "}
        {/* Increased z-index */}
        <div className="flex items-center">
          <img
            className="object-cover h-12 w-28 smd:h-12 smd:w-32"
            src={image2}
            alt="Store Logo"
          />
        </div>
          ...
          <div className="relative">
            ...
            {isBookingsOpen && (
              ...
              >
                ...
                <Link
                  to={"/AllTours"}
                  className="font-radios z-20 p-2 rounded-xl hover:shadow-lg hover:shadow-fade-black block px-2 py-2 text-sm text-white hover:text-blue-600 hover:bg-white lg:text-[14px] md:text-[12px]"
                >
                  Tour Booking{" "}
                </Link>
                <Link
