...
import image from "../image/Family Packages (10).png";
import SingleNavbar from "./SingleNavbar";
const PersonalPackage = () => {
  const [PersonalPackage, setPersonalPackage] = useState([]);
  ...
  useEffect(() => {
    if (tourPackage) {
      if (tourPackage.personalTours) {
        setPersonalPackage(tourPackage.personalTours.slice(0, 3));
      } else {
        console.warn("PersonalPackage not found in tourPackage");
      }
    }
  }, [tourPackage]);
  const BookTour = (id) => {
    ...
    >
      ...
      <div className="flex flex-col space-y-24 flex-wrap px-14 ml-16 justify-center items-center w-full lg:w-[90%] pb-6 ">
        ...
        <div className="flex flex-row md:mt-0 sssm:mt-72 gap-y-4 flex-wrap w-[100%] md:w-[80%]">
          {PersonalPackage.map((tour, index) => (
            <div
