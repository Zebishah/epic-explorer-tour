...
import { showHotelReviews } from "../Redux/Slices/ShowTourDetailsSlice";
const ReviewHotel = (id) => {
  const [Reviews, setReviews] = useState([]);
  const dispatch = useDispatch();
  const { HotelReviews } = useSelector((state) => state.TourDetail);

  useEffect(() => {
    dispatch(showHotelReviews(id));
  }, [dispatch]);

  useEffect(() => {
    if (HotelReviews) {
      if (HotelReviews.getReview) {
        setReviews(HotelReviews.getReview.slice(0, 3));
        console.log("HotelReviews", HotelReviews.getReview);
      }
    }
  }, [HotelReviews]);
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <div className="flex flex-col items-start">
                <h3 className="text-lg font-semibold font-radios">
                  {review.name}
                </h3>
