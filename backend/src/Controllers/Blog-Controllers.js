...
export const addBlog = async (req, res, next) => {
  ...
  if (existingBlog) {
    return res
      .status(400)
      .json({ success: false, message: "blog already existed " });
  }
  let blog;
  try {
    blog = new Blogs({
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Blog Added Successfully",
    Category: "Blog Added",
    message: `One Blog ${blog.name} is Added from our site`,
    date,
  });
  ...
  return res
    .status(200)
    .json({ success: true, message: "New blog is created", blog: blog });
};
export const getBlogs = async (req, res, next) => {
  ...
  if (!blogs) {
    ...
    return res
      .status(400)
      .json({ success, message: "no blogs found in database" });
  }

  return res
    .status(200)
    .json({ success: true, message: "here are your all blogs", blogs: blogs });
};
...
export const getTourBlogs = async (req, res, next) => {
  ...
  if (!tour) {
    ...
    return res
      .status(400)
      .json({ success, message: "no toursfound in database" });
  }
  let blogs;
  try {
...
export const deleteBlog = async (req, res, next) => {
  ...
  try {
    ...
    let date = new Date();
    let notificationAdmin = new NotificationsAdmin({
      accommodationName: blog.name,
      Category: "blog deleted",
      message: `blog ${blog.name} is deleted from our site`,
      date: date,
    });
    await notificationAdmin.save();
    ...
    let notificationUser = new NotificationsUser({
      user: "no",
      broadCast: "yes",
      accommodationName: "Blog Deleted Successfully",
      Category: "Blog Deleted",
      message: `${blog.name} Blog is Deleted Successfully`,
    });
    await notificationUser.save();
...
export const updateBlog = async (req, res, next) => {
  ...
  if (!blog) {
    return res
      .status(400)
      .json({ success: false, message: "blog not existed" });
  }
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: blog.name,
    Category: "blog is updated",
    message: `one blog ${blog.name} information is updated in our site`,
    date: date,
  });
  await notificationAdmin.save();
  ...
  return res
    .status(200)
    .json({ success: true, message: "blog updated successfully", blog: blog });
};
