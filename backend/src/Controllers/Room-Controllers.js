...
export const addRoom = async (req, res, next) => {
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Room Added Successfully",
    Category: "Room Added",
    message: `One Room ${name} is added to our site`,
    date,
  });
  ...
  return res
    .status(200)
    .json({ success: true, message: "New Room is created", Room: room });
};
...
export const deleteRoom = async (req, res, next) => {
  ...
  if (!deletedRoom) {
    return res.status(400).json({
      success: false,
      message: "Room not existed that u are trying to delete",
    });
  }
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Room deleted Successfully",
    Category: "Room deleted",
    message: `One Room ${deletedRoom.name} is deleted from our site`,
    date,
  });
  await notificationAdmin.save();
...
export const updateRoom = async (req, res, next) => {
  ...
  if (!room) {
    return res
      .status(400)
      .json({ success: false, message: "room not existed" });
  }
  ...
  let date = new Date();
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Room info Successfully",
    Category: "Room Updated",
    message: `${name} Room info updated successfully`,
    date,
  });
  await notificationAdmin.save();
  return res
    .status(200)
    .json({ success: true, message: "room updated successfully", room: room });
};
export const filterRooms = async (req, res, next) => {
  ...
  if (!filteredRooms || filteredRooms.length === 0) {
    return res.status(404).json({
      success: false,
      message: "No rooms found with the given filter",
    });
  }
...
export const openRoom = async (req, res, next) => {
  ...
  if (!room) {
    ...
    return res
      .status(400)
      .json({ success, message: "room not existed that u are trying to open" });
  }
  ...
  if (!roomServiceIt) {
    ...
    return res.status(400).json({
      success,
      message: "room services not existed that u are trying to open",
    });
  }

  return res
    .status(200)
    .json({ success: true, room: room, roomServiceIt: roomServiceIt });
};
...
export const getFormData = async (req, res, next) => {
  ...
  try {
    roomBookingCheck = await BookingHotel.findOne({
      roomId: room.id,
  ...
  if (roomBookingCheck) {
    ...
    return res
      .status(400)
      .json({ success, message: "room already existed in booking database" });
  }
  let roomBooking;
  room.prices = room.prices * days;
  try {
    roomBooking = new BookingHotel({
  ...
  return res.status(200).json({
    success: true,
    message: "New Room booking start do payment",
    bill: bill,
...
export const roomPayment = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      try {
        roomBooked = await BookHotel.findOne({
      ...
      try {
        ...
        roomBooked = new BookHotel({
          bookedRoomNo: newBookedRoomNo,
          hotelId: roomBooking.hotelId,
          image: roomBooking.image,
          roomName: roomBooking.roomName,
          roomId: roomBooking.roomId,
          roomPrice: roomBooking.roomPrice,
          roomType: roomBooking.roomType,
          checkInDate: roomBooking.checkInDate,
          bookerName: roomBooking.bookerName,
          bookerEmail: roomBooking.bookerEmail,
          bookerPhone: roomBooking.bookerPhone,
          bookerAddress: roomBooking.bookerAddress,
          suggestion: roomBooking.suggestion,
          bookerId: roomBooking.bookerId,
          members: roomBooking.members,
          days: roomBooking.days,
      ...
      if (roomHistory) {
        return res
          .status(400)
          .json({ success: false, message: "room already existed " });
      }
      ...
      try {
        ...
        roomHistory = new HotelBookingHistory({
          hotelId: roomBooking.hotelId,
          roomId: roomBooking.roomId,
          amount: roomBooking.roomPrice,
          roomName: roomBooking.roomName,
          bookingDate: roomBooking.checkInDate,
          bookerName: roomBooking.bookerName,
          bookerEmail: roomBooking.bookerEmail,
        });
      ...
      try {
        ...
        await BookingHotel.findOneAndDelete({
          roomId: roomBooking.roomId,
          bookerId: user.id,
        });
        await makingRoomBill.findOneAndDelete({
          bookerId: user.id,
          booking: roomBooking.id,
        });
      } catch (error) {
      ...
      try {
        delHotelBooking = await BookingHotel.findOneAndDelete({
          roomId: roomId,
        });
      } catch (error) {
        return next(error);
      ...
      try {
        delHotelBill = await makingRoomBill.findOneAndDelete({
          bookerId: user.id,
        });
      } catch (error) {
        return next(error);
      ...
      let notificationAdmin = new NotificationsAdmin({
        accommodationName: "Room Payment  Successfully",
        Category: "Room Payment",
        message: `one user ${roomHistory.bookerName} booked ${roomHistory.name} room from our site and did Payment successfully `,
        date: date,
      });
      await notificationAdmin.save();
      ...
      await notificationUser.save();
      const options = {
        email: roomBooked.bookerEmail,
        message: `You Booked ${roomBooked.roomName} Room Successfully Now You are Eligible for This Room. Your Status is Booking Successful and You Booked this Room From Us on Date:${roomBooked.checkInDate}`,
      };
      console.log(options);
...
export const countRooms = async (req, res, next) => {
  ...
  res.status(200).json({
    success,
    message: "here is your Room count",
    RoomCount: RoomCount,
  });
};
...
export const relatedRooms = async (req, res, next) => {
  let id = req.body.id;
  let rooms;
  try {
    rooms = await Room.findById(id);
  } catch (error) {
    return next(error);
  }
  ...
  if (!rooms) {
    return res
      .status(400)
      .json({ success: false, message: "no rooms found in database" });
  }
  let finalRoom = await Room.find({ type: rooms.type });

  return res.status(200).json({
    success: true,
    message: "here are your all rooms",
    rooms: finalRoom,
  });
};
