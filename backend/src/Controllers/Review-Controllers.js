...
export const addReviews = async (req, res, next) => {
  ...
  if (tourCheck) {
    try {
      existingReview = new Review({
        name,
        email,
        reviewedService: tourCheck.name,
        image: user.pic,
        words,
        rating,
      });
    ...
    if (!existingReview) {
      return res
        .status(400)
        .json({ success: false, message: "Please give valid Review...." });
    }
  }
  ...
  if (roomCheck) {
    try {
      existingReview = new HotelReviews({
        name,
        email,
        reviewedService: roomCheck.name,
        image: user.pic,
        words,
        rating,
      });
  ...
  if (transportCheck) {
    try {
      existingReview = new TransportReviews({
        name,
        email,
        reviewedService: transportCheck.carName,
        image: user.pic,
        words,
        rating,
      });
  ...
  if (!tourCheck && !transportCheck && !roomCheck) {
    return res
      ...
      .json({
        success: false,
        message: "Did'nt added review in accommodations ",
      });
  }
  let date = new Date();

  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Review received Successfully",
    Category: "Review received",
    message: `we received one review from ${name}`,
    date,
  });
  ...
  let notificationUser = new NotificationsUser({
    user: user.userName,
    broadCast: "no",
    accommodationName: "review Added Successfully",
...
export const getHotelReviews = async (req, res, next) => {
  ...
  if (!room) {
    success = false;
    return res.status(400).json({ success, message: "Did'nt get room...." });
  }

  let getReview;
  try {
    getReview = await HotelReviews.find({ reviewedService: room.name });
  } catch (error) {
    return next(error);
  }
  if (!getReview) {
...
export const getTransportReviews = async (req, res, next) => {
  ...
  if (!transport) {
    ...
    return res
      .status(400)
      .json({ success, message: "Did'nt get transport...." });
  }

  let getReview;
  try {
    getReview = await TransportReviews.find({
      reviewedService: transport.carName,
    });
  } catch (error) {
    return next(error);
  }
...
export const getUserHotelReviews = async (req, res, next) => {
  ...
  let getReview;
  try {
    getReview = await HotelReviews.find({
      reviewedService: id,
      email: user.email,
    });
  } catch (error) {
...
export const countReviews = async (req, res, next) => {
  ...
  res
    ...
    .json({
      success,
      message: "here is your Review count",
