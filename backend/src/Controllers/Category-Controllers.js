...
export const createCategory = async (req, res, next) => {
  ...
  if (name.trim() === "" || pic.trim() === "" || description === "") {
    return res
      .status(400)
      .json({ success: false, message: "enter ur credentials first" });
  }
...
export const showCategoryTours = async (req, res, next) => {
  ...
  if (!tour) {
    return res
      .status(400)
      .json({ success: false, message: "tour not created" });
  }
  return res
    .status(200)
    .json({ success: true, message: "here is ur category tours", tour });
};
