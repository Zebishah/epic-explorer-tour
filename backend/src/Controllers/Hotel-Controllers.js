...
export const addHotel = async (req, res, next) => {
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "Hotel Added Successfully",
    Category: "Hotel Added",
    message: `One Hotel ${name} is added to our site`,
    date,
  });
...
export const getHotel = async (req, res, next) => {
  ...
  if (!Hotel) {
    return res.status(400).json({
      success: false,
      message: "no Hotels found in database",
      statusCode: 400,
    });
  }
...
export const deleteHotel = async (req, res, next) => {
  ...
  if (!deleteHotel) {
    return res.status(400).json({
      success: false,
      message: "Hotel not existed that u are trying to delete",
      statusCode: 400,
    });
  }
  let date = new Date();
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "hotel deleted Successfully",
    Category: "hotel deleted",
    message: `One hotel ${deleteHotel.name} is deleted from our site`,
    date,
  });
  await notificationAdmin.save();
...
export const updateHotel = async (req, res, next) => {
  ...
  if (!Hotel) {
    return res
      .status(400)
      .json({ success: false, message: "Hotel not existed", statusCode: 400 });
  }
  ...
  let notificationAdmin = new NotificationsAdmin({
    accommodationName: "hotel updated Successfully",
    Category: "hotel updated",
    message: `One hotel ${Hotel.name} is updated in our site`,
    date,
  });
  await notificationAdmin.save();
...
export const countHotels = async (req, res, next) => {
  ...
  if (!HotelCount) {
    return res
      .status(400)
      .json({ success: false, message: "no Hotels found", statusCode: 400 });
  }

  res.status(200).json({
    success: true,
    message: "here is your Hotel count",
