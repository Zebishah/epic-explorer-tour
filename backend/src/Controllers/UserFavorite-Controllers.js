...
export const addUserFavorite = async (req, res, next) => {
  ...
  if (tourCheckFavrt) {
    Favrt = new UserFavrt({
      compId: tourCheckFavrt.id,
      accommodationName: "Tour",
      name: tourCheckFavrt.name,
      prices: tourCheckFavrt.price,
      description: tourCheckFavrt.description,
      image: tourCheckFavrt.image,
      userId: user.id,
    });
    await Favrt.save();
    ...
    let notificationUser = new NotificationsUser({
      user: user.userName,
      broadCast: "no",
      accommodationName: "Tour Added to Favorites",
      Category: "Tour Favorite",
      message: `${user.userName} ${tourCheckFavrt.name} Tour Added To Your Favorite`,
    });
    await notificationUser.save();
    return res
      .status(200)
      .json({ success: true, message: "here are your Favrt", Favrt: Favrt });
  }
  ...
  if (hotelCheckFavrt) {
    Favrt = new UserFavrt({
      compId: hotelCheckFavrt.id,
      accommodationName: "Hotel",
      name: hotelCheckFavrt.name,
      prices: hotelCheckFavrt.prices,
      description: hotelCheckFavrt.description,
      image: hotelCheckFavrt.image,
      userId: user.id,
    });
    await Favrt.save();
    user.wishList.push(id);
    ...
    let notificationUser = new NotificationsUser({
      user: user.userName,
      broadCast: "no",
      accommodationName: "Hotel Added to Favorites",
      Category: "Hotel Favorite",
      message: `${user.userName} ${hotelCheckFavrt.name} Hotel Added To Your Favorite`,
    });
    await notificationUser.save();
    return res
  ...
  if (transportCheckFavrt) {
    Favrt = new UserFavrt({
      compId: transportCheckFavrt.id,
      accommodationName: "Transport",
      name: transportCheckFavrt.name,
      prices: transportCheckFavrt.prices,
      description: transportCheckFavrt.description,
      image: transportCheckFavrt.image,
      userId: user.id,
    });
    await Favrt.save();
    user.wishList.push(id);
    ...
    let notificationUser = new NotificationsUser({
      user: user.userName,
      broadCast: "no",
      accommodationName: "Transport Added to Favorites",
      Category: "Transport Favorite",
      message: `${user.userName} ${transportCheckFavrt.name} Transport Added To Your Favorite`,
    });
    await notificationUser.save();
    return res
  ...
  if (!Favrt) {
    return res
      .status(400)
      .json({ success: false, message: "that accommodation not found" });
  }
};
...
export const getUserFavorite = async (req, res, next) => {
  ...
  if (!Favorites) {
    return res
      .status(400)
      .json({ success: false, message: "no Favorites are here" });
  }
  ...
  return res
    ...
    .json({
      success: true,
      message: "here are your all Favorites",
      Favorites: Favorites,
    });
};
...
export const deleteUserFavorite = async (req, res, next) => {
  ...
  let Favorites;
  try {
    Favorites = await UserFavrt.findOneAndDelete({
      compId: id,
      userId: user.id,
    });
  ...
  let notificationUser = new NotificationsUser({
    user: user.userName,
    broadCast: "no",
    accommodationName: `${Favorites.name} Favorite Deleted`,
    Category: `${Favorites.accommodationName} Favorite`,
    message: `${user.userName} ${Favorites.name} Added To Your Favorite`,
  });
  await notificationUser.save();
  return res
    ...
    .json({
      success: true,
      message: "here are your deleted Favorite",
      Favorites: Favorites,
    });
};
